...
import { Search, Plus, User as UserIcon, LogOut, Trash2, Shield } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold bg-gradient-to-r from-purple-600 via-pink-600 to-orange-500 bg-clip-text text-transparent tracking-tight">
                One Talk
              </h1>
            </div>
              ...
              {user ? (
                <>
                  ...
                  >
                    ...
                    {user.isAdmin && (
                      <span className="flex items-center gap-0.5 px-1.5 py-0.5 bg-red-500 text-white text-xs rounded-full" title="管理员">
      ...
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute top-1/2 left-1/2 w-24 h-24 bg-pink-200/20 rounded-full blur-xl animate-float-slow"></div>

        <div className="relative h-full flex flex-col items-center justify-center text-white px-4">
          <h2 className="text-4xl md:text-5xl font-bold mb-3 tracking-tight animate-fade-in-down drop-shadow-lg">
            有趣的人，都在这里
          </h2>
          <p className="text-lg md:text-xl text-white/90 mb-6 animate-fade-in-up drop-shadow">
            专业知识 · 生活服务 · 自由交流
          </p>
          <div className="flex gap-3 animate-fade-in-up">
            <span className="px-4 py-1.5 bg-white/20 backdrop-blur-sm rounded-full text-sm border border-white/30">
              ✨ 点赞互动
            </span>
            <span className="px-4 py-1.5 bg-white/20 backdrop-blur-sm rounded-full text-sm border border-white/30">
              🔥 实时热点
            ...
            {loading ? (
              ...
              <div className="text-center py-12 bg-white rounded-xl border">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">
                  {searchQuery ? '未找到相关帖子' : '暂无帖子'}
                </h3>
                <p className="text-gray-600">
                  {searchQuery ? '试试其他关键词' : user ? '快来发布第一个帖子吧！' : '登录后发布帖子'}
                </p>
                {!user && !searchQuery && (
                  <button
                    onClick={() => setShowAuthModal(true)}
      ...
      {selectedPost && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden animate-scale-in flex flex-col">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {canDeletePost(selectedPost) && (
                    <button
                      ...
                      title={user?.isAdmin && selectedPost.author?._id !== user.id ? '管理员删除' : '删除帖子'}
                    >
                ...
                {user && (
                  <form onSubmit={handleCommentSubmit} className="mb-6">
                    ...
                    <button
                      ...
                      className="mt-2 px-6 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg font-medium hover:from-purple-700 hover:to-pink-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      发表评论
                    </button>
                  </form>
